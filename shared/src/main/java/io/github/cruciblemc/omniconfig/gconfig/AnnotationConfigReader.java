...
public class AnnotationConfigReader {
    ...
    public IOmniconfig read() {
        ...
        this.getLoadingCallbacks(ConfigLoadCallback.Stage.BEFORE_INIT).stream().forEach(method -> this.tryInvoke(method, wrapper));

        wrapper.versioningPolicy(annotation.policy());
        wrapper.terminateNonInvokedKeys(annotation.terminateNonInvokedKeys());
        wrapper.loadFile();

        this.getLoadingCallbacks(ConfigLoadCallback.Stage.AFTER_INIT).stream().forEach(method -> this.tryInvoke(method, wrapper));
        this.loadFieldValues(wrapper);
        this.getLoadingCallbacks(ConfigLoadCallback.Stage.BEFORE_FINALIZATION).stream().forEach(method -> this.tryInvoke(method, wrapper));
