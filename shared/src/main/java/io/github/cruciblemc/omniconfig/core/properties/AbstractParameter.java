...
public abstract class AbstractParameter<T extends IAbstractProperty> implements IAbstractProperty {
    ...
    protected AbstractParameter(Builder<T, ?> builder) {
        ...
        this.isValidEnvironment = !this.sidedType.isSided() || this.sidedType.getSide() == OmniconfigAPI.getEnvironment();
    }
    ...
    protected void assertValidEnvironment() {
        if (!this.isValidEnvironment)
            throw new IllegalAccessError("Attempted to access config property " + this.getID()
                    + " of sided type " + this.sidedType
                    + " in invalid environment " + OmniconfigCore.INSTANCE.getEnvironment()
                    + "!");
    }
